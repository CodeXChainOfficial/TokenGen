...
const Createtokenhomepage: React.FC = () => {
  ...
  const handleMainnetNetworkChange = (
  ...
  const handleTestnetNetworkChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3004/api/tokenCount")
      .then((response) => {
  ...
  const handleCreateToken = async () => {
    ...
    try {
      const yourTokenContract = new web3.eth.Contract(
        abiArray,
        contractAddress
      );
      ...
      axios
        .post("http://localhost:3004/api/tokens", newTokenInfo)
        .then((response) => {
          if (response.status === 201) {
            console.log("Token information saved successfully.");
          } else {
            console.error(
              "Failed to save token information:",
              response.statusText
            );
          }
        })
      ...
      axios
        .post("http://localhost:3004/api/tokenCount", { count: newCount })
        .then((response) => {
          if (response.status === 201) {
      ...
      const response = await axios.get(
        `http://localhost:3004/api/tokens/${userAddress}`
      );
      if (response.status === 200) {
      ...
      const fetchDeployedTokens = async (address: string) => {
        try {
          const response = await axios.get(
            `http://localhost:3004/api/tokens/${address}`
          );
          if (response.status === 200) {
            setDeployedTokens(response.data);
          } else {
    ...
    <div>
      <div>
        ...
        <select
          ...
          onChange={handleMainnetNetworkChange}
        >
      ...
      <div>
        ...
        <select
          ...
          onChange={handleTestnetNetworkChange}
        >
      ...
      {deploymentHash && <p>Transaction Hash: {deploymentHash}</p>}
      <input
        type="text"
        value={tokenName}
